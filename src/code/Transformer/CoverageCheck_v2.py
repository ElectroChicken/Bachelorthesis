...
class CoverageCheck():
    ...
    def on_model_loop(self, model):
        ...
        for loop in self.loops:
            covered = True
            for atm in loop:
                if not self.atoms.intersection(self.heads[atm]):
                    covered = False
                    break
            if covered:
    ...
    def on_model_scc(self, model):
        ...
        for scc in self.sccs:
            ...
            if all(self.atoms.intersection(self.heads[atm]) for atm in scc):
                self.posCCov.add(tuple(scc))
            elif all(not self.atoms.intersection(self.heads[atm]) for atm in scc):
                self.negCCov.add(tuple(scc))
    ...
    def check_positive(self):
        ...
        for atm, body in self.heads.items():
            if self.atoms.intersection(body):
    ...
    def check_negative(self):
        ...
        for atm, body in self.heads.items():
            if not self.atoms.intersection(body):
    ...
    def check_coverage(self, program, testcases, verbose=True, loop="loop"):
        for file in testcases:
            ...
            loop="scc"
            if loop == "loop":
                ctl.solve(on_model=self.on_model_loop)
            elif loop == "scc":
                ctl.solve(on_model=self.on_model_scc)
            else:
    ...
    def print_pos_Lcoverage(self):
        ...
        for loop in self.loops:
            if loop not in self.posLCov:
                for atm in loop:
                    locations[loop].append(atm.location)
    ...
    def print_pos_Ccoverage(self):
        ...
        for scc in self.sccs:
            if scc not in self.posCCov:
                for atm in scc:
                    locations[tuple(scc)].append(atm.location)

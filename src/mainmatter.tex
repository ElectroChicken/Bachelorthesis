...
Finally we define the \emph{positive atom dependency graph} of a program $P$ as the directed graph \(G = (V, E)\) where the vertices are all the atoms occurring in the program and the edge \((a,b) \in E\) exists, iff there exists a rule \(r \in P\) such that \(a \in H(r) \ \text{and}\ b \in B^+(r)\) hold.
...
\label{def:input transforamtion}
    ...
    Firstly, add a choice rule to program $P$ "\(\{\_i_0; \ldots;\_i_n\} = 1.\)" where each $\_i_i$ is a label atom associated with the input of test case $T_i$. Secondly, For every test case \( T_j \in S\) and for every atom \( a_{j,i} \in \inp(T_j)\) add a rule to $P$ "\(a_{j,i} \leftarrow \_i_j.\)" where $\_i_j$ is the label atom corresponding to test case $T_j$.
\end{definition} 
...
\label{ex:information gathering}
    ...
    \begin{align*}
        ...
        d &\leftarrow b
    \end{align*}
    a simplified abstract syntax tree of the first rule can be seen in \cref{fig:ast example}. The atom dictionary for this program will look like this after information gathering:
    
    \code{atoms = \{(c, 0): [[0], [location], [0]], (d, 0): [[1], [location], [1]], (a, 0): [[2], [location], []], (b, 0): [[3], [location], []]\}}

    Where the first value is the atom label, the second describes the location of the atom in the program and the third value is the label of the rule it is defined in. Atoms $a$ and $b$ have an empty list as last value since they are not defined in this program.
\end{example}
...
In the previous section, four main suggestions were made on how to adjust some definitions in order to make calculating coverage for further program classes possible. The first one is to only consider non ground atoms instead of ground atoms and the second was a change to the basic coverage function schema. These two together make handling variables possible. The third adjustment is a slight change in the definition of the defining rules of an atom $a$ \(\Def(a)\). And finally the definition of the positive atom dependency graph was adjusted. In this section we want to discus how to make these changes work with our proposed syntactic transformations and thus allow the computation of coverage for almost all types of answer set programs.
